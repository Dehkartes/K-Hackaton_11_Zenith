...
conn = pymysql.connect(host='127.0.0.1', user='Admin', password='1592', db='k-hackaton_11', charset='utf8')
cur = conn.cursor()

def Record(CCTV_ID: int, Crackdown_Type: int, Date: str, Locatioin_Id: int):
	cur.execute(f'insert into crackdown(CCTV_ID, Location_ID, Date, Crackdown_Type) values(\'{CCTV_ID}\', \'{Locatioin_Id}\', \'{Date}\', \'{Crackdown_Type}\')')

Record(1, 1, '20200125', 4)
conn.commit()
